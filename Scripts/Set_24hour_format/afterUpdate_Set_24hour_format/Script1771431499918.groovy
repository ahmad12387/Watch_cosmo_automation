...
// ========== STEP 10: Click on Clock format ==========
tapNodeCenter(xml, "resource-id", "io.senlab.cosmo:id/prefClock")

// ========== STEP 11: Refresh XML on Clock Format screen ==========
xml = dumpAndParseXML()

//// ========== STEP 12: Select 24 hour radio button ==========
//tapNodeCenter(xml, "resource-id", "io.senlab.cosmo:id/rb01")

// ========== STEP 13: Verify 24 hour radio button is selected ==========
xml = dumpAndParseXML()

def rb24 = xml.depthFirst().find {
	it.@'resource-id'.toString() == 'io.senlab.cosmo:id/rb01'
}

if (!rb24) {
	KeywordUtil.markFailed("❌ 24 hour radio button not found")
} else if (rb24.@checked.toString() == 'true') {
	KeywordUtil.logInfo("✅ 24 hour clock format is successfully selected")
} else {
	KeywordUtil.markFailed("❌ 24 hour clock format is NOT selected")
}
