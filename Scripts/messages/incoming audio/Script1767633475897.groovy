...
// ===================================================================
// STEP 4: Verify Incoming Audio Message Presence
// ===================================================================
KeywordUtil.logInfo("=== STEP 4: Verifying Incoming Audio Message ===")

def audioNode = chatXml.depthFirst().find {
	it.@'resource-id'.toString() == 'io.senlab.cosmo:id/msg_audio'
}

if (audioNode) {
	KeywordUtil.markPassed("✅ Incoming audio message FOUND in chat.")
} else {
	KeywordUtil.markFailed("❌ Incoming audio message NOT FOUND in chat.")
}


// ===================================================================
// STEP 5: Save Incoming Audio Data (Local Storage)
// ===================================================================
if (audioNode) {

    String audioBounds = audioNode.@bounds.toString()

    // Move up to the message container
    def messageContainer = audioNode.parent().parent()

    def timeNode = messageContainer.depthFirst().find {
        it.@'resource-id'.toString() == 'io.senlab.cosmo:id/msg_time'
    }

    def durationNode = messageContainer.depthFirst().find {
        it.@'resource-id'.toString() == 'io.senlab.cosmo:id/audioTimeLeft'
    }

    String audioTime = timeNode ? timeNode.@text.toString() : "UNKNOWN"
    String audioDuration = durationNode ? durationNode.@text.toString() : "UNKNOWN"

    KeywordUtil.logInfo("🔊 AUDIO FOUND")
    KeywordUtil.logInfo("🕒 Time: ${audioTime}")
    KeywordUtil.logInfo("⏱ Duration: ${audioDuration}")
    KeywordUtil.logInfo("📐 Bounds: ${audioBounds}")
}
