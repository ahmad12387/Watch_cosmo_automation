...
// ===================================================================
// STEP 6: Scroll Chat & Collect All Text Messages (DEDUPED + REPORT)
// ===================================================================
KeywordUtil.logInfo("=== STEP 6: Scrolling chat and collecting all text messages ===")

Map<String, String> textLogMap = new LinkedHashMap<>()
...
for (int scroll = 0; scroll < maxScrolls; scroll++) {
    ...
    textMsgNodes.each { msgNode ->
        ...
        // ⚠️ RENAMED VARIABLE (was timeNode)
        ...
        // Deduplication key
        String dedupKey = "${msgText}|${msgTimeVal}"

        String logEntry =
            "👤 ${contactName} | 💬 '${msgText}' | 🕒 ${msgTimeVal} | 📅 ${visibleDate}"

        // Replace only if date was missing before
        if (!textLogMap.containsKey(dedupKey)
            || textLogMap[dedupKey].contains("Date not visible")) {

            textLogMap[dedupKey] = logEntry
        }
    }

    // Stop if nothing new is found
    if (textLogMap.size() == previousCount) {
        KeywordUtil.logInfo("📌 No new text messages detected — reached top of chat.")
        break
    }

    previousCount = textLogMap.size()

    // Working scroll gesture
...
// 📋 FINAL REPORT
// =======================
KeywordUtil.logInfo("=== 📋 UNIQUE TEXT MESSAGES FOUND (${textLogMap.size()}) ===")

textLogMap.values().eachWithIndex { entry, i ->
    KeywordUtil.logInfo("(${i + 1}) ${entry}")
}

if (textLogMap.isEmpty()) {
