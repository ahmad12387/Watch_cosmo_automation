...
// ===================================================================
// STEP 2: Tap Contact → “hez ron” (dynamic)
// ===================================================================
KeywordUtil.logInfo("=== STEP 2: Tapping Contact 'hez ron' Dynamically ===")

def xml2 = dumpAndParseXML()

tapNodeCenter(xml2, "text", "hez ron")
...
// ===================================================================
// STEP 4: Verify Incoming Video Message Presence
// ===================================================================
KeywordUtil.logInfo("=== STEP 4: Verifying Incoming Video Message ===")

def videoNode = chatXml.depthFirst().find {
	it.@'resource-id'.toString() == 'io.senlab.cosmo:id/msg_video' ||
	it.@'resource-id'.toString() == 'io.senlab.cosmo:id/msg_video_with_icon'
}

if (videoNode) {
	KeywordUtil.markPassed("✅ Incoming video message FOUND in chat.")
} else {
	KeywordUtil.markFailed("❌ Incoming video message NOT FOUND in chat.")
}


// ===================================================================
// STEP 5: Save Incoming Video Data (Local Storage)
// ===================================================================
if (videoNode) {

	String videoBounds = videoNode.@bounds.toString()

	def timeNode = chatXml.depthFirst().find {
		it.@'resource-id'.toString() == 'io.senlab.cosmo:id/msg_time'
	}

	String videoTime = timeNode ? timeNode.@text.toString() : "UNKNOWN"

	KeywordUtil.logInfo("🎥 VIDEO FOUND")
	KeywordUtil.logInfo("🕒 Time: ${videoTime}")
	KeywordUtil.logInfo("📐 Bounds: ${videoBounds}")
}
