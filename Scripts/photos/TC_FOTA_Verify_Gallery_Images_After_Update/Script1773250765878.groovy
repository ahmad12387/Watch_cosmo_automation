...
//Step — Function to Get All Gallery Images
...
//Step 3 — After FOTA Collect Images Again
...
//Step 4 — Load Saved Images
File file = new File("C:\\cosmo_xml\\gallery_before.txt")

def savedImages = file.readLines()

//Step 5 — Compare
if(savedImages == imagesAfter) {
	
		KeywordUtil.markPassed("✅ All gallery images preserved after FOTA")
	
	}
	else {
	
		KeywordUtil.markFailed("❌ Gallery images changed after FOTA")
	
	}
	
	
	// ===============================================================
	// FINAL SUMMARY: Display gallery image count and bounds
	// ===============================================================
	KeywordUtil.logInfo("========== GALLERY SUMMARY AFTER FOTA ==========")
	
	KeywordUtil.logInfo("Total Images Present: " + imagesAfter.size())
	
	imagesAfter.eachWithIndex { bounds, index ->
		KeywordUtil.logInfo("Image ${index + 1} Bounds: ${bounds}")
	}
